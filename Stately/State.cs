...
namespace Stately {
	public class State {
        ...
        List<AnyStateTransition> anyStateTransitions = new List<AnyStateTransition>();
        ...
        // keeps a map from "from states" to transitions so we dont have to loop over all the transitions
        Dictionary<State, List<Transition>> fromToTransitions = new Dictionary<State, List<Transition>>();
		...
		public Transition ChangeTo(State state)
		{
            ...
            AddFromTransition(this, transition);
			return transition;
		}

        void AddFromTransition(State fromState, Transition transition)
        {
            if (!fromToTransitions.ContainsKey(fromState))
            {
                fromToTransitions[fromState] = new List<Transition>();
            }

            fromToTransitions[fromState].Add(transition);
        }
		...
		void UpdateTransitions(float deltaTime)
		{
            ...
            for (var i = 0; i < anyStateTransitions.Count; i++)
			{
		...
		void CheckTransitions()
		{
			if (currentSubState != null)
			{
                if (currentSubState.PotentialTransitionsExist(currentSubState))
                {
                    var potentialTransitions = currentSubState.PotentialTransitions(currentSubState);
                    for (var i = 0; i < potentialTransitions.Count; i++)
                    {
                        var transition = potentialTransitions[i];
        ...
        bool PotentialTransitionsExist(State fromState)
        {
            return fromToTransitions.ContainsKey(fromState);
        }

        List<Transition> PotentialTransitions(State fromState)
        {
            return fromToTransitions[fromState];
        }
